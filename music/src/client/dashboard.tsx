...
const MusicPlayer = () => {
  ...
  // Reset progress khi chuyển bài (chỉ khi index thay đổi)
  useEffect(() => {
    setProgress(0);
    setDuration(0);
  }, [currentSongIndex]);
    ...
    <div className="relative max-w-120 mx-auto animate-slideUp">
      ...
      <div className="flex flex-col items-center gap-5">
        ...
        <div className="w-full max-w-120 px-5 pt-6 pb-4 bg-linear-to-br from-[rgba(126,217,87,0.1)] to-[rgba(0,188,212,0.1)] backdrop-blur-xl border border-white/10 shadow-[0_8px_32px_rgba(126,217,87,0.3)] rounded-b-[30px] z-10">
          ...
          {/* CD */}
          <div className="flex flex-col items-center my-5 w-55 mx-auto relative">
            <div
              ...
              style={{
                backgroundImage: `url('${currentSong.imageUrl || "https://tse3.mm.bing.net/th/id/OIP.lucx6lfHqnK0P6dzh6-t0wAAAA?w=180&h=180&rs=1&pid=ImgDetMain&o=7&rm=3"}')`,
              }}
            >
            ...
            <div
              className={`flex items-center justify-center gap-1 mt-5 h-10 ${isPlaying ? "playing" : ""} border-transparent bg-transparent`}
            >
              {[...Array(15)].map((_, i) => (
                <span
                  key={i}
                  className={`w-1 h-3 bg-linear-to-t from-[#7ed957] to-[#00bcd4] rounded-sm ${isPlaying ? "animate-wave-bar bar" + (i + 1) : ""}`}
                />
              ))}
            </div>
          </div>
          ...
          <div className="flex items-center justify-around py-5">
            <button
              ...
              className={`p-4 text-lg rounded-full transition-all hover:text-[#7ed957] hover:scale-110 hover:bg-[rgba(126,217,87,0.1)] ${isRepeat ? "text-[#7ed957] bg-[rgba(126,217,87,0.2)]" : "text-[#a8a8a8]"}`}
            >
              <FaRedo />
            </button>
            <button
            ...
            <button
              ...
              className={`p-4 text-lg rounded-full transition-all hover:text-[#7ed957] hover:scale-110 hover:bg-[rgba(126,217,87,0.1)] ${isRandom ? "text-[#7ed957] bg-[rgba(126,217,87,0.2)]" : "text-[#a8a8a8]"}`}
            >
              <FaRandom />
            </button>
          </div>
          ...
          {songs.map((song, index) => (
            ...
            >
              <div
                ...
                style={{
                  backgroundImage: `url('${song.imageUrl || "https://via.placeholder.com/50?text=Song"}')`,
                }}
              />
              <div className="flex-1 px-4">
                <div
                  className={`text-base font-semibold mb-1 ${index === currentSongIndex ? "text-[#1a1a2e]" : "text-white"}`}
                >
                  {song.name}
                </div>
                <div
