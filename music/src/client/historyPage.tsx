...
import { FaHistory, FaPlay, FaPause, FaHeart, FaRegHeart, FaEllipsisH } from 'react-icons/fa';
...
const HistoryPage = () => {
  ...
  return (
    <div className="p-6 md:p-10 space-y-8 animate-fade-in custom-scrollbar">
      {/* Header Section */}
      <section className="relative overflow-hidden rounded-[40px] group">
        ...
        <div className="glass-panel-3d border-0 p-8 md:p-12 relative z-10 flex items-center gap-8">
          <div className="w-20 h-20 md:w-28 md:h-28 rounded-3xl bg-gradient-to-br from-amber-50 to-orange-600 flex items-center justify-center text-white shadow-2xl shadow-orange-500/20">
            <MdHistory size={48} />
          </div>
          <div className="space-y-2">
            <h1 className="text-4xl md:text-5xl font-black text-white">Gần <span className="text-amber-500">Đây</span></h1>
            <p className="text-slate-400 font-medium">{filteredSongs.length} bài hát nghe gần đây</p>
          </div>
        </div>
      </section>
      ...
      {songs.length > 0 && (
        <div className="max-w-5xl mx-auto">
          <input
            type="text"
            placeholder="Tìm kiếm bài hát, nghệ sĩ trong lịch sử nghe..."
            value={searchQuery}
            onChange={(e) => { setSearchQuery(e.target.value); setCurrentPage(1); }}
            className="w-full bg-white/5 border border-white/10 rounded-2xl px-5 py-4 text-white focus:outline-none focus:border-amber-500/50 focus:ring-4 focus:ring-amber-500/5 transition-all text-sm font-semibold placeholder-slate-500"
          />
        </div>
      )}
