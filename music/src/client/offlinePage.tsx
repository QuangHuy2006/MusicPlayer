...
const OfflinePage = () => {
  ...
  const handlePlaySong = async (song: Song) => {
    // When playing a song on the offline page, let's load it from IndexedDB
    const dbSong = await offlineDb.getSong(song.id);
    if (dbSong && dbSong.audioBlob) {
      // Create a temporary object URL that the audio element will use
      const objectUrl = URL.createObjectURL(dbSong.audioBlob);
      
      // Let's create an offline playing song object
      const playingSong: Song = {
        ...song,
        url: objectUrl, // Substitute URL with local blob URL
      };
      
      // If we had cover image stored, resolve it too
      if (dbSong.imageBlob) {
        playingSong.imageUrl = URL.createObjectURL(dbSong.imageBlob);
      }

      playSong(playingSong, offlineSongs);
    } else {
      // Fallback
