...
const AddSongPopup = ({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!name || !file) {
      setMessage({ text: 'Vui lòng nhập tên bài hát và chọn file nhạc', type: 'error' });
      return;
    }
    setLoading(true);
    setMessage(null);
    ...
    try {
      ...
      if (data.success) {
        setMessage({
          text: isPrivate 
            ? 'Thêm nhạc riêng tư thành công!' 
            : 'Thêm nhạc thành công! Đang chờ quản trị viên duyệt.', 
          type: 'success'
        });
        setTimeout(() => {
          resetForm();
        ...
        <div className="flex-1 p-8 md:p-10 bg-[#0a0a0a]/40 backdrop-blur-xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4 pt-2">
              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-500 uppercase tracking-widest">File Nhạc</label>
