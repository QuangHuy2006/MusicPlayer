      ...
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center gap-4 mb-10">
          <div className="w-16 h-16 rounded-2xl premium-card flex items-center justify-center shadow-[var(--shadow-3d-out)]">
            <FaHistory className="text-[var(--accent-gold)] text-3xl" />
          </div>
          <div>
            <h2 className="text-3xl font-bold text-white tracking-tight drop-shadow-md">Nghe gần đây</h2>
            <p className="text-[var(--accent-blue)] font-medium mt-1">{songs.length} bài hát đã nghe</p>
          </div>
        </div>

        {songs.length === 0 ? (
          <div className="text-center py-20 glass-panel-3d">
            <FaHistory className="text-6xl text-[var(--accent-gold)] opacity-50 mx-auto mb-4" />
            <h3 className="text-xl font-bold text-white">Chưa có lịch sử nghe</h3>
                ...
                >
                  <div className="relative w-16 h-16 rounded-xl overflow-hidden shrink-0 shadow-lg group-hover:shadow-[var(--shadow-3d-out)] transition-shadow">
                    ...
                    <div className={`absolute inset-0 bg-black/40 flex items-center justify-center transition-opacity ${isActive ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
                      {isActive && isPlaying ? (
                        <FaPause className="text-[var(--accent-gold)] text-lg drop-shadow-md" onClick={(e) => { e.stopPropagation(); togglePlay(); }} />
                      ) : (
                        <FaPlay className="text-[var(--accent-gold)] text-lg drop-shadow-md ml-1" />
                      )}
                    </div>
                  </div>

                  <div className="flex-1 min-w-0 pr-4">
                    <h3 className={`text-base font-bold truncate transition-colors drop-shadow-sm ${isActive ? 'text-[var(--accent-gold)]' : 'text-slate-200 group-hover:text-[var(--accent-blue)]'}`}>
                      {song.name}
                    </h3>
